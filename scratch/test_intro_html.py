...
def main():
    ...
    try:
        ...
        # Try to find cafe_main iframe and switch to it first
        ...
        # Switch to iframe if we clicked the button, otherwise we are already there at top-level
        if btn:
            crawler.driver.switch_to.default_content()
            crawler._switch_to_cafe_iframe()
            
        print("Waiting 5 seconds for intro page to load...")
        time.sleep(5.0)
            
        # Save page source
        page_src = crawler.driver.page_source
        output_file = "scratch/intro_iframe_source.html"
        with open(output_file, "w", encoding="utf-8") as f:
            f.write(page_src)
        print(f"Saved page source to {output_file} (length: {len(page_src)})")
        
        # Search for campingfirst manager name
        if "캠핑퍼스트매니저" in page_src:
            print("Found '캠핑퍼스트매니저'!")
            idx = page_src.index("캠핑퍼스트매니저")
            start = max(0, idx - 400)
            end = min(len(page_src), idx + 400)
            print("--- Snippet ---")
            print(page_src[start:end])
            print("---------------")
        else:
            print("Could not find '캠핑퍼스트매니저' in page source.")
