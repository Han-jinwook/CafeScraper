...
with st.sidebar:
                ...
                if not st.session_state.commenter or not st.session_state.commenter.driver:
                    ...
                    with st.spinner("수집 중..."):
                        ...
                        articles = st.session_state.commenter.scrape_board_list(
                            direct_board_url, start_dt, end_dt, 
                        ...
                        if articles:
                            ...
                            df = pd.read_sql_query("SELECT post_id, nickname, title, date, url, board_name FROM posts ORDER BY created_at DESC LIMIT 1000", conn)
                            conn.close()
                            st.session_state.target_df = df
                        else:
                            st.warning("수집 실패")
    ...
    # 2. 댓글 작성 섹션
    # --------------------------------------
    st.subheader("2. 댓글 내용")
    st.session_state.template_list = load_templates()
    selected_template = st.selectbox("템플릿", st.session_state.template_list, label_visibility="collapsed")
    
    default_text = "" if selected_template == "(직접 입력)" else selected_template
    final_template = st.text_area("댓글 내용 입력", value=default_text, height=150, help="{닉네임}, {제목} 치환 가능")
    ...
    if st.button("🚀 댓글 작성 시작", type="primary", use_container_width=True):
        if not st.session_state.commenter or not st.session_state.commenter.driver:
            st.error("브라우저 미실행")
        elif "target_df" not in st.session_state or st.session_state.target_df is None or st.session_state.target_df.empty:
            st.error("타겟 없음")
...
# 메인 화면 (데이터 확인 및 로그)
# ==========================================
st.title("🤖 자동 댓글러 (Auto Commenter)")
...
if st.session_state.target_df is not None and not st.session_state.target_df.empty:
    ...
    st.info("👈 사이드바에서 타겟을 검색하거나 불러와주세요.")
