...
class NaverCafeCrawler:
    ...
    def scrape_board_list(
        ...
        def _read_page_date_range(page_no: int) -> tuple[Optional[datetime], Optional[datetime], int, str]:
            ...
            try:
                ...
                rows_inner = self.driver.find_elements(
                    By.CSS_SELECTOR,
                    "div[class*='ArticleItem'], li[class*='article'], li[class*='ArticleItem'], li[class*='Article'], tr[class*='article'], div.article-board table tbody tr, table[class*='Article'] tbody tr, #article-list li, .article-board > ul > li, #main-area table tbody tr, table tbody tr"
                )
                if not rows_inner:
                    return None, None, 0, ""
        ...
        def _auto_locate_start_page(initial_page: int) -> int:
            ...
            if te < p1_min_d:
                ...
                while left <= right:
                    ...
                    m_max_d = m_max.date() if isinstance(m_max, datetime) else m_max
                    if te < m_min_d:
                        left = mid + 1
                    elif te > m_max_d:
                        found = mid
                ...
                jump_page = max(1, int(found) - 1)
                self._update_status(
                    f"✅ 해당 기간의 시작 지점을 찾았습니다. 안전을 위해 {jump_page}페이지부터 수집을 시작합니다."
                )
                return jump_page
    ...
    def extract_cafe_leaders(self, cafe_url_or_name: str) -> List[Dict[str, str]]:
        """
        특정 카페의 운영진(매니저/스태프) 네이버 ID 및 이메일 리스트를 수집합니다.
        """
        if not self.driver:
        ...
        club_id = None
        try:
            club_id = self.driver.execute_script("return (window.g_sClubId || null);")
        except:
            pass
        ...
        # 2. 카페 소개(CafeIntro) 버튼 클릭을 통해 iframe 내에 인트로 로드
        self.driver.switch_to.default_content()
        btn = None
        ...
        if btn:
            try:
                ...
                self._update_status(f"[운영진 추출] 소개 버튼 클릭 에러: {e_click}")
        else:
            intro_url = f"https://cafe.naver.com/CafeIntro.nhn?clubid={club_id}"
            self.driver.get(intro_url)
            time.sleep(3.0)
            
        # 3. iframe 전환 및 자바스크립트로 스태프 리스트 수집 (DOM 주입 void 리턴 우회 기법)
        self.driver.switch_to.default_content()
        switched = self._switch_to_cafe_iframe()
        if not switched:
            self._update_status("[운영진 추출] 카페소개 iframe 전환에 실패했습니다.")
            return []
        ...
        leaders_data = []
        if raw_string_data:
            records = raw_string_data.strip("|").split("|")
            for rec in records:
                if not rec:
                    continue
                parts = rec.split("^")
        ...
        self._update_status(f"[운영진 추출] 발견된 운영진 수: {len(leaders_data)}명. 레이어 ID 역추출을 개시합니다.")
        
        # 4. 각 닉네임 엘리먼트를 클릭하여 실제 네이버 ID 획득
        results = []
        for item in leaders_data:
            ...
            try:
                a_el = self.driver.find_element(By.ID, id_attr)
                naver_id = self._get_member_id_via_layer(a_el)
                if naver_id and naver_id != "unknown":
                    ...
                    self._update_status(f"  └ [실패] {nick} ({role}) -> ID 복구 실패 (unknown)")
            except Exception as e_el:
                self._update_status(f"  └ [에러] {nick} ID 획득 실패: {e_el}")
    ...
    def send_automatic_memo(self, target_ids: List[str], subject: str, content: str, delay_range: tuple, status_callback=None) -> tuple[int, int]:
        """
        추출된 네이버 ID 리스트를 대상으로 딜레이를 두고 쪽지를 자동 발송합니다.
        """
        if not self.driver:
            raise Exception("브라우저가 실행되지 않았습니다.")
        ...
        _log(f"[쪽지 발송 개시] 총 대상: {len(target_ids)}명")
        ...
        for idx, target_id in enumerate(target_ids):
            ...
            _log(f"({idx+1}/{len(target_ids)}) [{target_id}] 쪽지 발송 시도...")
            
            try:
                # 쪽지 쓰기 바로가기 URL 이동
                memo_url = f"https://note.naver.com/note/write.nhn?targetUserId={target_id}"
                self.driver.get(memo_url)
                time.sleep(2.0)
                ...
                try:
                    title_input = WebDriverWait(self.driver, 5).until(
                        EC.presence_of_element_located((By.CSS_SELECTOR, "input[name='title'], #title"))
                    )
                    title_input.clear()
                    title_input.send_keys(subject)
                except:
                    pass # 제목 폼이 없는 구조일 수도 있음
                    
                # 쪽지 내용 입력
                content_textarea = WebDriverWait(self.driver, 5).until(
                    EC.presence_of_element_located((By.CSS_SELECTOR, "textarea[name='noteContent'], #noteContent, textarea.write_area"))
                )
                ...
                send_btn = WebDriverWait(self.driver, 5).until(
                    EC.element_to_be_clickable((By.CSS_SELECTOR, "button.btn_send, #btn_send, a.btn_send"))
                )
                ...
                success_count += 1
                _log(f"  => [성공] {target_id} 쪽지 전송 완료")
            except Exception as e:
                fail_count += 1
                _log(f"  => [실패] {target_id} 쪽지 전송 실패: {e}")
                
            # 슬로우 딜레이
            if idx < len(target_ids) - 1:
                delay = random.uniform(delay_range[0], delay_range[1])
                _log(f"  [대기] {delay:.1f}초 동안 지연 대기 중...")
                time.sleep(delay)
                
        _log(f"[쪽지 발송 완료] 성공: {success_count}건, 실패: {fail_count}건")
        return success_count, fail_count

    def send_automatic_email(self, smtp_server: str, port: int, user_id: str, user_pw: str, target_emails: List[str], subject: str, content: str, delay_range: tuple, status_callback=None) -> tuple[int, int]:
        """
        SMTP 서버 연동을 통해 추출된 이메일 리스트로 메일을 자동 발송합니다.
        """
        import smtplib
        ...
        _log(f"[이메일 발송 개시] SMTP: {smtp_server}:{port} | 계정: {user_id}")
        ...
        for idx, email in enumerate(target_emails):
            ...
            _log(f"({idx+1}/{len(target_emails)}) [{email}] 메일 발송 시도...")
            
            try:
                msg = MIMEMultipart()
                sender_email = f"{user_id}@naver.com" if "naver" in smtp_server.lower() and "@" not in user_id else user_id
                ...
                success_count += 1
                _log(f"  => [성공] {email} 메일 발송 완료")
            except Exception as e:
                fail_count += 1
                _log(f"  => [실패] {email} 메일 발송 에러: {e}")
                
            # 슬로우 딜레이
            if idx < len(target_emails) - 1:
                delay = random.uniform(delay_range[0], delay_range[1])
                _log(f"  [대기] {delay:.1f}초 동안 지연 대기 중...")
                time.sleep(delay)
                
        _log(f"[이메일 발송 완료] 성공: {success_count}건, 실패: {fail_count}건")
        return success_count, fail_count
